...
__all__ = ['make_quadratic', 'make_quadratic_cqm', \
           'reduce_binary_polynomial', 'make_quadratic_alt'] # line modified by haoyu
...
def reduce_binary_polynomial_alt(poly: BinaryPolynomial) -> Tuple[
        List[Tuple[FrozenSet[Hashable], Number]],
        List[Tuple[FrozenSet[Hashable], Hashable, Tuple[Number, Number]]]
    ]:
    """Reduce a binary polynomial to linear and quadratic terms, plus constraints.
    ...
    Modified from reduce_binary_polynomial() by haoyu in Sep., 2022.
    ...
    Returns:
        Two-tuple of a list of terms and their biases, as tuples, and a list of
        the original, auxiliary variables and a tuple of the lower and higher
        bounds of the sum of original terms, as a tuple.
    ...
    while idx:
        ...
        # ----Block added by haoyu----
        upperbound = 0
        lowerbound = 0
        for bias in terms.values():
            if bias > 0:
                upperbound += bias
            elif bias < 0:
                lowerbound += bias
        ...
        prod_var = _new_product(variables, *pair)
        constraints.append((pair, prod_var, (lowerbound, upperbound))) # line modified by haoyu
        prod_var_set = {prod_var}
...
def make_quadratic_alt(poly: Union[Polynomial, BinaryPolynomial],
                   vartype: Optional[Vartype] = None,
                   bqm: Optional[BinaryQuadraticModel] = None) -> BinaryQuadraticModel:
    """Create a binary quadratic model from a higher order polynomial.
    The energy penalty for violating the product constraint is automatically chosen
    according to the coefficients of the reduced terms.

    Modified from make_quadratic() by haoyu in Sep., 2022.
    ...
    """
    from dimod.generators import and_gate, and_gate_pos

    bqm, vartype = _init_quadratic_model(bqm, vartype, BinaryQuadraticModel)
    poly = _init_binary_polynomial(poly, vartype)

    variables = set().union(*poly)
    reduced_terms, constraints = reduce_binary_polynomial_alt(poly)

    for (u, v), p, (l, r) in constraints:

        # add a constraint enforcing the relationship between p == u*v
        if vartype is Vartype.BINARY:
            # ----Block modified by haoyu
            if l >= 0:
                constraint = and_gate_pos(u, v, p)
            else:
                constraint = and_gate(u, v, p)
            # ----Block modified by haoyu
            bqm.info['reduction'][(u, v)] = {'product': p, 'range': (l, r)} # line modified by haoyu
        elif vartype is Vartype.SPIN:
            aux = _new_aux(variables, u, v)  # need an aux in SPIN-space
            constraint = _spin_product([u, v, p, aux])
            bqm.info['reduction'][(u, v)] = {'product': p, 'auxiliary': aux, 'range': (l, r)} # line modified by haoyu
        else:
            raise RuntimeError("unknown vartype: {!r}".format(vartype))

        # scale constraint and update the polynomial with it
        constraint.scale(max(-l, r)) # line modified by haoyu
        for v, bias in constraint.linear.items():
            bqm.add_variable(v, bias)
        for uv, bias in constraint.quadratic.items():
            bqm.add_interaction(*uv, bias)
