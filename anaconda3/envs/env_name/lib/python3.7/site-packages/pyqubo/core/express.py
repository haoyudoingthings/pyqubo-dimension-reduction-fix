...
class Express:
    ...
    def compile(self, strength=None): # line modified by haoyu
        ...
        Args:
            strength (float): The strength of the reduction constraint.
                ...
                If not specified, a minimal sufficient value will be chosen automatically.
        ...
        def binary_product(multiplier, multiplicand, product, weight=1): # line modified by haoyu
            return {
        ...
        # Constraint for AND(multiplier, multiplicand) = product
        # but only when the coefficient is positive
        def binary_product_pos(multiplier, multiplicand, product, weight=1):
            return {
                BinaryProd({product}): weight,
                BinaryProd({multiplicand, product}): -weight,
                BinaryProd({multiplier, product}): -weight,
                BinaryProd({multiplier, multiplicand}): weight
            }
        # ----Block added by haoyu----
        ...
        if strength is not None:
            ...
            bqm = dimod.make_quadratic_alt(pubo, dimod.BINARY)
        # ----Block modified by haoyu----
        ...
        for (a, b), v in bqm.info['reduction'].items():
            ...
            if strength is not None:
                product_consts["AND({},{})={}".format(a, b, prod)]\
                    = binary_product(a, b, prod, weight=strength)
            else:
                l, r = normalize_label(v['range'])
                if l >= 0:
                    product_consts["AND({},{})={}".format(a, b, prod)]\
                        = binary_product_pos(a, b, prod, weight=max(-l, r))
                else:
                    product_consts["AND({},{})={}".format(a, b, prod)]\
                        = binary_product(a, b, prod, weight=max(-l, r))
            # ----Block modified by haoyu----
